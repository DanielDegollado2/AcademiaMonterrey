...
LAT=$(awk -v ms="$LAT_MS" 'BEGIN{printf "%.3f", ms/1000}')
...
medir() {
  ...
  wait
  fin=$(date +%s.%N)
  printf '  %-12s %3d peticiones en \033[1m%6.2f s\033[0m\n' "$nombre" "$N" "$(awk -v a="$fin" -v b="$ini" 'BEGIN{printf "%.2f", a-b}')"
}

esperado_bloq=$(awk -v n="$N" -v c="$NUCLEOS" -v l="$LAT" 'BEGIN{printf "%.2f", (n/c)*l}')
tandas=$(awk -v n="$N" -v c="$NUCLEOS" 'BEGIN{printf "%.1f", n/c}')
...
cat <<TXT
  ...
  Prediccion antes de correrlo:
    ...
    bloqueante  -> ~${esperado_bloq}s   ($N peticiones / $NUCLEOS hilos = ${tandas} tandas de ${LAT}s)
...
cat <<TXT
  ...
  Y ojo: el bloqueante no es un endpoint raro que inventamos. Es